...
import com.lijunyu.spring.util.Utils;
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					...
					File folder = new File("/home/chenjie/CJHadoopOnline/"
							+ user.getUsername());
					if (!folder.exists()) {
						boolean flag = folder.mkdir();//如果文件不目录存在，则在服务器本地创建
						if(folder.mkdir()){
...
//							return "NO";
	 ...
	 public static final String HDFS = "hdfs://127.0.0.1:9001";
	...
	public void wordcount(HttpServletRequest request,
		...
		String input = HDFS + "/user/" + username
				+ "/wordcountinput";//指定Hadoop文件系统的输入文件夹
		String output = HDFS + "/user/" + username
				+ "/wordcountoutput";//指定Hadoop文件系统的输出文件夹
		String reslt = output + "/part-00000";//默认输出文件
		try {
			Thread.sleep(3*1000);
			c.main(new String[] { input, output, reslt });//调用单词统计任务
...
//	        conf.addResource("classpath:/hadoop/core-site.xml");//添加Hadoop配置，找到Hadoop部署信息
//	        conf.addResource("classpath:/hadoop/hdfs-site.xml");//Hadoop配置，找到文件系统
//	        conf.addResource("classpath:/hadoop/mapred-site.xml");
	        JobConf conf = new JobConf(HadoopController.class);
	        conf.setJobName("HadoopController");
	        conf.addResource("classpath:/hadoop/core-site.xml");
	        conf.addResource("classpath:/hadoop/hdfs-site.xml");
	        conf.addResource("classpath:/hadoop/mapred-site.xml");
	        
	        
			FileSystem fileSystem = FileSystem.get(URI.create(HDFS),conf);//得打文件系统
...
//		}
		...
		//展示文件加下内容
        Path path = new Path("/user/" + user.getUsername()
		+ "/wordcountinput/" + file.getName());
        FileStatus[] list = fileSystem.listStatus(path);
        System.out.println("ls: " + hadoopFile);
        System.out.println("==========================================================");
